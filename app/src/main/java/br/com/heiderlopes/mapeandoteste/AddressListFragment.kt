...
class AddressListFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        return AlertDialog.Builder(requireContext())
            ...
            .setOnDismissListener { viewModel.clearSearchAddressResult() }.create()
    }

    companion object {
        private const val EXTRA_ADDRESSES = "addresses"
        fun newInstance(addresses: List<Address>):

                AddressListFragment {
            return AddressListFragment().apply {
                arguments =
                    Bundle().apply { putParcelableArray(EXTRA_ADDRESSES, addresses.toTypedArray()) }
            }
        }
